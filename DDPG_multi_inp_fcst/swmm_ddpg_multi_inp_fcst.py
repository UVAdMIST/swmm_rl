...
repeats = 5  # number of consecutive times to train on single .inp file
...
for file in os.scandir(os.path.join(data_dir, "syn_inp_train")):
    if file.name.endswith('.inp'):
        ...
        if file_num == 1:
            ...
            agent.save_weights('swmm_rl_multi_inp_forecast/agent_weights/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()

        else:
            agent.load_weights('swmm_rl_multi_inp_forecast/agent_weights/ddpg_swmm_weights.h5f')
            agent.fit(env, nb_steps=train_steps, verbose=0)
            agent.save_weights('swmm_rl_multi_inp_forecast/agent_weights/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()
...
for file in os.scandir(os.path.join(data_dir, "syn_inp_test")):
    if file.name.endswith('.inp'):
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig("C:/Users/Ben Bowes/PycharmProjects/swmm_rl_multi_inp_forecast/plots_weighted_rwd/"
