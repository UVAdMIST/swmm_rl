...
from swmm_rl_multi_inp.swmm_Model_multi_inp import BasicEnv
...
file_num = 1
for file in os.scandir("C:/Users/Ben Bowes/PycharmProjects/LongTerm_SWMM/syn_inp_train"):
    if file.name.endswith('.inp'):
        ...
        if file_num == 1:
            ...
            agent.save_weights('swmm_rl_multi_inp/agent_weights/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()

        else:
            agent.load_weights('swmm_rl_multi_inp/agent_weights/ddpg_swmm_weights.h5f')
            agent.fit(env, nb_steps=train_steps, verbose=0)
            agent.save_weights('swmm_rl_multi_inp/agent_weights/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()
...
for file in os.scandir("C:/Users/Ben Bowes/PycharmProjects/LongTerm_SWMM/syn_inp_test"):
    if file.name.endswith('.inp'):
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig("C:/Users/Ben Bowes/PycharmProjects/swmm_rl_multi_inp/plots_weighted_rwd/" + file.name.split('.')[0] + ".png",
                    dpi=300)
        plt.close()
