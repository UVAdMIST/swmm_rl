...
from swmm_rl_multi_inp_fcst_gated.swmm_Model_multi_inpfcst_gate import BasicEnv

depth = 4.61  # maximum depth of ponds
repeats = 1  # number of consecutive times to train on single .inp file
data_dir = "C:/Users/Ben Bowes/PycharmProjects/LongTerm_SWMM"
start_time = datetime.now()

# loop through training envs
file_num = 1
for file in os.scandir(os.path.join(data_dir, "syn_inp_train_gated")):
    if file.name.endswith('.inp'):
        ...
        if file_num == 1:
            ...
            agent.save_weights('swmm_rl_multi_inp_fcst_gated/agent_weights/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()

        else:
            agent.load_weights('swmm_rl_multi_inp_fcst_gated/agent_weights/ddpg_swmm_weights.h5f')
            agent.fit(env, nb_steps=train_steps, verbose=0)
            agent.save_weights('swmm_rl_multi_inp_fcst_gated/agent_weights/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()
...
for file in os.scandir(os.path.join(data_dir, "syn_inp_test_gated")):
    if file.name.endswith('.inp'):
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig("C:/Users/Ben Bowes/PycharmProjects/swmm_rl_multi_inp_fcst_gated/plots_weighted_rwd/"
                    + file.name.split('.')[0] + ".png", dpi=300)
        plt.close()
