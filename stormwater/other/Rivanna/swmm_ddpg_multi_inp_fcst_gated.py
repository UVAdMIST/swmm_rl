...
from swmm_Model_multi_inp_fcst import BasicEnv
...
for file in os.scandir(os.path.join(data_dir, "syn_inp_train_gated")):
    if file.name.endswith('.inp'):
        ...
        if file_num == 1:
            ...
            agent.save_weights('/home/bdb3m/swmm_rl/agent_weights_fcst_gated/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()

        else:
            agent.load_weights('/home/bdb3m/swmm_rl/agent_weights_fcst_gated/ddpg_swmm_weights.h5f')
            agent.fit(env, nb_steps=train_steps, verbose=0)
            agent.save_weights('/home/bdb3m/swmm_rl/agent_weights_fcst_gated/ddpg_swmm_weights.h5f', overwrite=True)
            env.close()
...
for file in os.scandir(os.path.join(data_dir, "syn_inp_test_gated")):
    if file.name.endswith('.inp'):
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig("/home/bdb3m/swmm_rl/plots_fcst_gated/" + file.name.split('.')[0] + ".png",
                    dpi=300)
        plt.close()
